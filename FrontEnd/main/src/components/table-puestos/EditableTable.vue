...
let puestos = ref<Puesto[]>([]);

onMounted(() => {
    fetch('http://127.0.0.1:8000/gimnasio/api/v1puestos/')
        .then(res => res.json())
        .then((datos: Puesto[]) => {
            puestos.value = datos; // Actualiza el valor de puestos
            console.log(puestos.value); // Imprime el valor de puestos
        })
        .catch(error => {
            console.error(error);
        });
...
const formTitle = computed(() => {
    return editedIndex.value === -1 ? 'Nuevo Puesto' : 'Editar Puesto';
});
</script>
<template>
    <v-row>
        <v-col cols="12" lg="4" md="6">
            <v-text-field density="compact" v-model="search" label="Buscar Puesto" hide-details variant="outlined"></v-text-field>
        </v-col>
        <v-col cols="12" lg="8" md="6" class="text-right">
            <v-dialog v-model="dialog" max-width="500">
                <template v-slot:activator="{ props }">
                    <v-btn color="primary" v-bind="props" flat class="ml-auto">
                        <v-icon class="mr-2">mdi-account-multiple-plus</v-icon>Agregar Puesto
                    </v-btn>
                </template>
                <v-card>
    ...
    <v-table class="mt-5">
        <thead>
            <tr>
                ...
                <th class="text-subtitle-1 font-weight-semibold">Descripcion del Puesto</th>
                <th class="text-subtitle-1 font-weight-semibold">Acciones</th>
            </tr>
        </thead>
        <tbody>
    <tr v-for="puesto in puestos" :key="puesto.ID">
        <td class="text-subtitle-1">{{ puesto.ID }}</td>
        <td>
            <div class="d-flex align-center py-4">
                <div class="ml-5">
                    <h4 class="text-h6">{{ puesto.nombre_Puesto }}</h4>
                </div>
            </div>
        </td>
        <td class="text-subtitle-1">{{ puesto.descripcion_Puesto }}</td>
        <td>
            <div class="d-flex align-center">
                <v-tooltip text="Edit">
                    <template v-slot:activator="{ props }">
                        <v-btn icon flat @click="editItem(puesto)" v-bind="props"
                ...
                <v-tooltip text="Delete">
                    <template v-slot:activator="{ props }">
                        <v-btn icon flat @click="deleteItem(puesto)" v-bind="props"
